...
export default function Upload({ onPrediction, isApiOnline }: UploadProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col items-center justify-center text-center space-y-3">
          ...
          `}>
            {isLoading ? (
              ...
              <UploadIcon className="w-8 h-8 text-primary-300 group-hover:text-accent-500 transition-colors" />
            )}
          </div>

          <div>
            <p className="text-primary-200 font-medium mb-1 truncate px-2">
      ...
      {/* Info message */}
      {!isApiOnline && (
        <div className="bg-amber-500/10 border border-amber-500/50 rounded-lg p-3 text-amber-400 text-sm flex items-start gap-2">
          <svg className="w-5 h-5 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          <div>
            <p className="font-semibold mb-1">API Offline</p>
            <p className="text-xs break-all">Inicie com: <code className="font-mono bg-amber-500/20 px-1 py-0.5 rounded">python app.py</code></p>
          </div>
        </div>
      )}
    </div>
